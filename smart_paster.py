...
TOOLS_ROOT = os.path.dirname(os.path.dirname(SCRIPT_DIR))
if TOOLS_ROOT not in sys.path:
    sys.path.insert(0, TOOLS_ROOT)

# Try to import from _shared (preferred)
try:
    from _shared.files import (
        generate_visual_tree as _shared_generate_visual_tree,
        build_clipboard_content as _shared_build_clipboard_content,
        get_language_hint,
        PatternMatcher,
    )
    from _shared.files.exclusion_patterns import DEFAULT_EXCLUSION_PATTERNS
    from _shared.extract import extract_pdf_text, extract_odt_text
    _USE_SHARED = True
except ImportError:
    _USE_SHARED = False
    # Local fallbacks
...
# Lazy imports for PDF/ODT (only needed if _shared not available)
_fitz = None
_odf_text = None
_odf_teletype = None
_odf_load = None
...
def _get_odf():
    global _odf_text, _odf_teletype, _odf_load
    if _odf_text is None:
        try:
            from odf import text, teletype
            from odf.opendocument import load as odf_load_func
            _odf_text = text
            _odf_teletype = teletype
            _odf_load = odf_load_func
        except ImportError:
            pass
    return _odf_text, _odf_teletype, _odf_load
...
def generate_visual_tree(
    ...
    if _USE_SHARED:
        return _shared_generate_visual_tree(directory, exclusion_regex, pattern_matcher)
...
def process_smart_request(
    user_request: str,
    directory: str,
    pattern_matcher: Optional[PatternMatcher] = None,
    cached_file_list: Optional[List[str]] = None,
) -> List[str]:
    """
    ...
    Args:
        ...
        cached_file_list: Optional pre-scanned list of relative file paths to skip os.walk
    """
    # Ensure the base directory is an absolute path for reliable comparisons
    base_directory = os.path.abspath(directory)

    if cached_file_list is not None:
        all_project_files = list(cached_file_list)
    else:
        # Create default matcher if none provided
...
def build_clipboard_content(file_paths: List[str], root_directory: str, max_size: Optional[int] = None, append_tree: bool = False, exclusion_regex: Optional[re.Pattern] = None) -> str:
    """Build clipboard content from file paths.

    Uses shared implementation if available, otherwise falls back to local.
    """
    # Use shared implementation if available
    if _USE_SHARED:
        return _shared_build_clipboard_content(
            file_paths, root_directory, max_size, append_tree, exclusion_regex
        )
    ...
    for i, abs_path in enumerate(file_paths):
        ...
        try:
            ...
            if ext in {'.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp'}:
                ...
                odf_text, odf_teletype, odf_load = _get_odf()
                if odf_text and odf_teletype and odf_load:
                    doc = odf_load(abs_path)
                    all_paragraphs = doc.getElementsByType(odf_text.P)
                    content = "\n".join(odf_teletype.extractText(p) for p in all_paragraphs)
                    block = f"# {rel_path}\n```text\n{content.strip()}\n```"
                else:
                    block = f"# {rel_path}\n```\n[ODT - odfpy not installed]\n```"
            elif ext == '.csv':
                ...
                if is_text:
                    with open(abs_path, 'r', encoding='utf-8', errors='replace') as f:
                        content = f.read()
                    lang_hint = _get_language_hint(rel_path)
                    block = f"# {rel_path}\n```{lang_hint}\n{content.strip()}\n```"
...
def _get_language_hint(filename: str) -> str:
    ...
    if _USE_SHARED:
        return get_language_hint(filename)
